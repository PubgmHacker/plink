...
const su = await api('/auth/signup', { method: 'POST', body: { email: `${tag}@test.plink`, password: `Passw0rd!${tag}`, username: `${tag}user` } });
...
console.log('ticket:', t.status, Object.keys(t.json ?? {}), 'protocol field:', t.json?.protocol?.length);

const ws = new WebSocket(`${WS_BASE}/ws/room/${roomObj.id}`, ['plink.v2', `plink.ticket.${t.json.ticket}`]);
...
setTimeout(() => {
  ...
  try { ws.send(JSON.stringify({ type: 'sync.command', protocolVersion: 2, roomId: roomObj.id, actionId: `${tag}-a1`, mediaId: null, positionMs: 1000, playing: true, rate: 1 })); } catch (e) { console.log('send failed:', e.message); }
}, 1000);
