...
async function wsConnect(label, roomId, token) {
  ...
  const ws = new WebSocket(`${WS_BASE}/ws/room/${roomId}`, ['plink.v2', `plink.ticket.${t.json.ticket}`]);
  const messages = [];
  ws.on('message', (d) => {
    const s = d.toString();
    console.log(`[${label}] MSG: ${s.slice(0, 160)}`);
    try { messages.push(JSON.parse(s)); } catch {}
  });
  ws.on('close', (code, reason) => console.log(`[${label}] CLOSE: ${code} ${reason}`));
  ws.on('error', (e) => console.log(`[${label}] ERROR: ${e.message}`));
  await new Promise((res, rej) => { ws.once('open', res); ws.once('error', rej); setTimeout(() => rej(new Error('timeout')), 8000); });
...
const room = await api('/rooms', { method: 'POST', token: host.token, body: { name: `${tag} room` } });
...
for (let i = 0; i < 40; i++) {
  if (A.messages.some((m) => m.type === 'session.ready') && B.messages.some((m) => m.type === 'session.ready')) break;
  await wait(250);
}
