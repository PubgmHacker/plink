...
///
/// Аудит 26.07.2026 P1: автодействие — ТОЛЬКО для контента (message/room).
...
async function liftRestriction(
  ...
  try {
    if (targetType === 'message') {
      ...
      const clearBan =
        !revertingReport || (revertingReport.resolution === 'ban' && revertingReport.resolvedAt !== null)
...
export async function moderationRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: { targetType: TargetType; targetId: string; reason: Reason; comment?: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply.code(201).send({ id: report.id, dueAt, slaHours: SLA_HOURS, autoHidden: hidden })
    },
  )
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      return reply.send({
        ...
        overdue: reports.filter((r: { dueAt: Date | null }) => r.dueAt !== null && r.dueAt.getTime() < now).length,
        reports,
      })
    },
  )
  ...
  fastify.post<{ Params: { id: string }; Body: { action: 'dismiss' | 'hide' | 'ban' | 'unban' } }>(
    ...
    async (request, reply) => {
      ...
      if (action === 'dismiss') {
        ...
        //
        // Аудит 26.07.2026 P2: раньше dismiss снимал его безусловно —
