...
export async function grantWebPremium(
  userId: string,
  plan: WebPlanId,
  paymentId: string,
  ...
  fastify.post(
    ...
    async (request: any, reply: any) => {
      if (!webPayConfigured()) {
        return reply
          .status(503)
          .send({ error: 'Оплата на сайте пока не подключена. Оформите Plink+ в приложении.' });
      }
      ...
      if (created.status >= 300 || !confirmationUrl) {
        request.log.error(
          { ykStatus: created.status, yk: created.json?.type, desc: created.json?.description },
          'yookassa create failed',
        );
    ...
    reply.send({
      ...
      plans: Object.fromEntries(
        Object.entries(WEB_PLANS).map(([id, p]) => [
          id,
          { title: p.title, price: p.price, days: p.days },
        ]),
      ),
    });
  });
