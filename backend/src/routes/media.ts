...
export default async function mediaRoutes(fastify, _options) {
  ...
  fastify.get(
    ...
    async (request: any, reply: any) => {
      ...
      // v2 marks the keyless web-search contract. Do not serve an older
      // Data-API response after switching the provider strategy.
      const shape = ['w', 'r', vkToken ? 'v' : ''].join('');
      const cacheKey = `media:search:v2:${shape}:${q}:${parsedLimit}`;
      ...
      try {
        const { results, providers, failed, counts } = await searchAllProviders(q, parsedLimit, {
          // Search the public YouTube page, not Data API search.list. The latter
          // spends project quota on every keystroke and eventually makes the beta
          // look offline. The key is used only as a provider-local fallback if
          // YouTube serves an anti-bot/consent page from this egress IP.
          useYouTubeWeb: true,
        ...
        request.log.info({ q, limit: parsedLimit, counts, failed }, 'media search providers');

        if (!providers.length) {
          request.log.error({ failed }, 'media search: no provider answered');
          return reply.status(502).send({ error: 'Search upstream unavailable' });
        }
        for (const f of failed) {
          if (f.reason.endsWith('not configured')) continue;
          request.log.warn(
            { provider: f.provider, reason: f.reason },
            'media search provider failed',
          );
        }
        ...
        request.log.error({ err: e }, 'media search failed');
  ...
  // ═════════════════════════════════════════════════════════════════════════
  // ── POST /api/media/stream-token — short-lived scoped token (P1 audit) ──
  // The stream proxy authenticates via ?token= (AVPlayer drops headers on
  // Range requests). Query strings leak into logs/proxies, so clients now
  // request this dedicated 45-minute token scoped to media streaming instead
  // of exposing the long-lived session JWT.
  fastify.post(
    '/media/stream-token',
    {
      preHandler: [fastify.authenticate],
