  ...
  fastify.post(
    ...
    async (request, reply) => {
      // P1 5.11: бесплатный тариф — AI_FREE_DAILY_LIMIT запросов в сутки.
      {
        ...
        const now = new Date();
        const premiumActive =
          !!requester?.isPremium && (!requester.premiumUntil || requester.premiumUntil > now);
      ...
      try {
        ...
        if (
          AI_ACTIONS_ENABLED &&
          lowerMsg.includes('созда') &&
          (lowerMsg.includes('комнат') || lowerMsg.includes('room'))
        ) {
          ...
          const mediaItem: { id: string; title: string; streamURL: string; source: string } =
            (context.mediaItem as
              { id?: string; title?: string; streamURL?: string; source?: string } | undefined) &&
            typeof context.mediaItem === 'object'
              ? {
                  id: String((context.mediaItem as any).id ?? 'youtube_search'),
                  title: String(
                    (context.mediaItem as any).title ?? context.title ?? 'AI suggested content',
                  ),
        ...
        const wantsQueue =
          /очеред|поставь|добавь|queue/.test(lastUserText) || lowerMsg.includes('очеред');
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (!OPENROUTER_API_KEY) {
        return reply
          .status(503)
          .send({ error: 'AI not configured. Set OPENROUTER_API_KEY env var.' });
      }
      ...
      const rawSince =
        typeof body.sinceMs === 'number' && Number.isFinite(body.sinceMs)
          ? body.sinceMs
          : nowMs - 30 * 60_000;
      ...
      try {
        ...
        /* mediaItem бывает строкой не-JSON — тогда без названия */
      }
        ...
        const response = await fetch(OPENROUTER_URL, {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: 'You are a movie/TV recommendation expert. Return JSON only.',
              },
...
`;
  ...
  switch (mode) {
    case 'room_host':
      return (
        base +
        `The user is hosting a watch party. Help them with:
- Setting up the room
...
Context: ${JSON.stringify(context || {})}`
    ...
    case 'movie_search':
      return (
        base +
        `The user is looking for something to watch. Help them:
...
Context: ${JSON.stringify(context || {})}`
    ...
    default:
      return (
        base +
        `Help the user with any question about Plink, movies, or general chat. Be helpful and friendly.`
      );
  }
}
