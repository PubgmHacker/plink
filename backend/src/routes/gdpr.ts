...
export default async function gdprRoutes(fastify) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const { password, twofaSecret, twofaBackupCodes, fcmToken, ...safeData } = userData as any;
      ...
      reply.header('Content-Disposition', `attachment; filename="plink-data-${userId}.json"`).send({
