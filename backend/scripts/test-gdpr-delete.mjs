...
async function main() {
  ...
  const room = await prisma.room.create({
    data: {
      name: `${tag} room`,
      hostID: u1.id,
      hostName: u1.username,
      code: tag.slice(-6).toUpperCase(),
    },
  });
  ...
  await prisma.directMessage.create({
    data: { senderID: u1.id, receiverID: u2.id, content: 'dm' },
  });
  await prisma.directMessage.create({
    data: { senderID: u2.id, receiverID: u1.id, content: 'dm2' },
  });
  ...
  const leftovers = {
    ...
    dms: await prisma.directMessage.count({
      where: { OR: [{ senderID: u1.id }, { receiverID: u1.id }] },
    }),
    friendships: await prisma.friendship.count({
      where: { OR: [{ userID: u1.id }, { friendID: u1.id }] },
    }),
