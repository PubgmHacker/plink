...
enum PlinkBrandPalette {
    ...
    /// Кромка тёмного хвоста B. У фигуры A светлый рим был с самого начала,
    /// у B — нет, и на почти чёрном сплэше хвост исчезал: замер по
    /// 18-splash.png дал долька/грунт 1.45:1 слева и 1.37:1 справа, то есть
    /// половина двухчастного знака физически не читается (порог для
    /// графики — 3:1). Заливку B не трогаем, она снята 1:1 с эталона; вместо
    /// перекраски обводим хвост тем же приёмом, что уже есть у A. Ось —
    /// собственный градиент B, так кромка гаснет в ту же сторону, что и тело.
    static var rimB: LinearGradient {
        LinearGradient(
            colors: [
                Color(red: 234/255, green: 223/255, blue: 255/255).opacity(0.62),
                Color(red: 234/255, green: 223/255, blue: 255/255).opacity(0.52)
            ],
            startPoint: markBStart, endPoint: markBEnd
        )
    }
    /// Хвост толще кромки A: у A под обводкой светлое тело, и штрих там —
    /// блик, а у B обводка остаётся единственной границей с фоном.
    /// Затухание мягкое (0.62 → 0.52), а не как у A: ось градиента B ведёт
    /// в нижний левый угол, то есть ровно на внешнюю дугу, и сильное
    /// затухание гасило кромку там, где она нужнее всего — на замере кончик
    /// капли давал 2.55:1 против 4.3:1 в середине дуги.
    static let rimBWidthRatio: CGFloat = 5.4 / 349.90
