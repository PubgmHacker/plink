...
struct PlinkBrandMark: View {
    ...
    /// Содержимое экрана: закат над горизонтом.
    ///
    /// ПОЧЕМУ НЕ ЛОГОТИП СЕРВИСА (решение 04.08.2026)
    ///
    /// Напрашивается положить на экран знак YouTube/VK/Rutube — «показываем,
    /// что умеем». Так делать нельзя по трём причинам, и первая закрывающая:
    ///
    ///   • Это чужие товарные знаки. Логотип стороннего сервиса внутри НАШЕГО
    ///     фирменного знака — прямое указание на несуществующее партнёрство;
    ///     App Review 5.2.5 запрещает подразумевать одобрение или связь с
    ///     сервисом, которой нет.
    ///   • Знак стал бы принадлежать одному сервису — та же ошибка, из-за
    ///     которой мы только что убрали синий. Plink работает с несколькими
    ///     площадками, и ни одна не должна оказаться «главной».
    ///   • Логотип пришлось бы менять при каждом изменении набора сервисов и
    ///     при каждом их ребрендинге.
    ///
    /// Нужен не конкретный контент, а ЛЮБОЙ узнаваемый кадр. Закат над
    /// горизонтом — самый читаемый сюжет на площади 41×24 pt: две зоны и
    /// диск. Он мгновенно говорит «идёт фильм», не называя ни одного фильма,
    /// и остаётся тёплым (свет экрана — постоянный цвет бренда).
    private func screenFace(s: CGFloat) -> some View {
        ZStack(alignment: .bottom) {
            // Небо: от белёсого у горизонта к тёплому вверху.
            LinearGradient(
                colors: [PlinkBrand.warmHot, Color(hex: 0xFFC978)],
                startPoint: .top,
                endPoint: .bottom
            )

            // Солнце над линией горизонта. Смещение больше высоты полосы
            // земли (0.055): при равном значении диск ложился ровно НА
            // кромку и срезался ею наполовину — читался обрезком, а не
            // солнцем.
            Circle()
                .fill(Color.white.opacity(0.95))
                .frame(width: s * 0.070, height: s * 0.070)
                .offset(y: -s * 0.088)

            // Земля — плотная полоса цветом темноты зала: она и создаёт
            // «кадр», отделяя небо от переднего плана.
            Rectangle()
                .fill(PlinkBrand.ink.opacity(0.88))
                .frame(height: s * 0.055)
        }
    }
